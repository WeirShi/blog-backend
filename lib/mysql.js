...
const userRegist = (params) => {
    ...
    const _sql = `insert into users (mobile, password, create_time) values("${mobile}", "${password}", NOW());`;
    return query(_sql);
}

// 根据手机号查找用户
const queryUserWithMobile = (mobile) => {
    const _sql = `select * from users where mobile = "${mobile}";`;
    return query(_sql);
}
...
const addCategory = (params) => {
    ...
    let _sql = `insert into categories (name, sort, create_time) values ("${name}", ${sort}, NOW());`
    return query(_sql);
}
...
const deleteCategory = (params) => {
    const { id } = params;
    let _sql = `update categories set is_delete = 1, update_time=NOW() where id = ${id};`;
    return query(_sql);
}
...
const updateCategory = (params) => {
    ...
    let _sql = `update categories set name = "${name}", sort = ${sort}, update_time=NOW() where id = ${id} and is_delete = 0;`;
    return query(_sql);
}

// 查找某一分类
const getCategoryDetail = (id) => {
    let _sql = `select * from categories where id = ${id};`
    return query(_sql);
}
...
const addTag = (params) => {
    ...
    let _sql = `insert into tags (name, sort, color, create_time) values ("${name}", ${sort}, "${color}", NOW());`
    return query(_sql);
}
...
const deleteTag = (params) => {
    const { id } = params;
    let _sql = `update tags set is_delete = 1, update_time=NOW() where id = ${id};`;
    return query(_sql);
}
...
const updateTag = (params) => {
    ...
    let _sql = `update tags set name = "${name}", sort = ${sort}, color = "${color}", update_time=NOW() where id = ${id} and is_delete = 0;`;
    return query(_sql);
}

// 查找某一标签
const getTagDetail = (id) => {
    let _sql = `select * from tags where id = ${id};`
    return query(_sql);
}
...
const addArticle = (params) => {
    ...
    let _sql = `insert into articles (title, content, cover, tags, categories, is_drafts, create_time) values("${title}", "${content}", "${cover}", "${tags}", "${categories}", ${is_drafts || 0}, NOW());`;
    return query(_sql);
}
...
const isPublishArticle = (params) => {
    ...
    let _sql = `update articles set is_publish = ${is_publish}, update_time=NOW()${is_publish === 1 ? ', publish_time=NOW()' : ''} where id = ${id} and is_delete = 0;`;
    return query(_sql);
}
